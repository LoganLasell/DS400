...
summarize <- summarize_gamma(7/3, 1/3)
```

1.  The most common value of λ is 10 and the mean is 12.

```{r}
plot_gamma(6, 0.5, mean = TRUE, mode = TRUE)
summarize <- summarize_gamma(6,0.5)
```

1.  The most common value of λ is 5, the variance is 3.

```{r}
plot_gamma(10.2, 1.8, mean = TRUE, mode = TRUE)
summarize <- summarize_gamma(10.2, 1.8)
```
...
summarize_gamma(1, 0.25)
```

1.  Why is the Poisson model a reasonable choice for our data Yi?

    -   This is count data, so it fits the dtype for gamma_poisson model

    -   whereas beta_model is for percentages of data

    -   generally fits our prior understanding of how many goals can be scored in a soccer game, usually between 0-2
...
summarize <- summarize_gamma_poisson(shape = 1, rate = 0.25, sum_y = 146 , n = 52)
```
