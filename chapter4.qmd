...
```{r}
plot_beta_binomial(alpha = 5, beta = 11, y = 11, n = 20)
```

The Clueless: Use `plot_beta_binomial()` to visualize the posterior

```{r}
plot_beta_binomial(alpha = 1, beta = 1, y = 11, n = 20)
```

The Optimist: Use `plot_beta_binomial()` to visualize the posterior

```{r}
plot_beta_binomial(alpha = 14, beta = 1, y = 11, n = 20)
```
...
1.  All prior choices are informative.
    ...
    1.  False, some prior choices are not informative as they can be vague and wash out posterior probability.

2.  There may be good reasons for having an informative prior.

    1.  True, a previous belief on how or why something happens - especially one without data to support it - could be the reason for an informative prior.

3.  Any prior choice can be overcome by enough data.

    1.  False, if the prior is vague enough, it could dominate the data.

4.  The frequentist paradigm is totally objective.

    1.  False, not accounting for new information leads to inevitable bias in stats.
...
########NOTE FROM LOGAN, CANT FIND IMAGE ON GITHUB########
...
#data is more influential to posterior
...
# prior is more influential to posterior
```
...
#compromise between influence of data and prior on posterior
```
...
# data has more influence on posterior
```
