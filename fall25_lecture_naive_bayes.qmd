...
-   Penguin has a bill length of 50 mm
...
```{r}
ggplot(pulse_of_the_nation, aes(x = age, y = income, color = climate_change)) +
  geom_point(alpha = 0.5)
```

```{r}
climate_model <- naiveBayes(climate_change ~ ., 
                            data = pulse_of_the_nation)
```
...
```{r}
climate_model_2 <- naiveBayes(climate_change ~ vaccines_are_safe + books,
                              data = pulse_of_the_nation)
```
...
```{r}
our_person <- data.frame(vaccines_are_safe = "Strongly Disagree", books = 1)
```

```{r}
predict(climate_model_2, newdata = our_person, type = "raw" )
...
```{r}
pulse_group <- pulse_of_the_nation %>% 
  group_by(vaccines_are_safe) %>% 
  summarize(
    mean_books = mean(books, na.rm = TRUE),
    sd_books = sd(books, na.rm = TRUE),
    n = n()
  )
```
...
```{r}
pulse_group %>% 
  count(vaccines_are_safe)
```

```{r}
# Strongly Disagree
dnorm(0.814, mean = 2.7, sd = 2.3)
likelihood_strongly_disagree <- 0.1239289
```
