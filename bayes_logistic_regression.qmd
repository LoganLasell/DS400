...
```{r}
# proportion of sample bookings canceled
 canceled_bookings <- 0.37
```
...
1.  What proportion of the sample bookings were canceled?
    ...
    1.  37% of bookings were canceled.

2.  Exploratory Data Analysis: Construct and discuss plots of `is_canceled` vs each of the four potential predictors above

    ```{r}
    ggplot(hotel_data, aes(x = average_daily_rate, y = lead_time, color = is_canceled )) +
      geom_point(alpha = 0.6) +
      geom_smooth()
    ```

    ```{r}
ggplot(hotel_data, aes(x = lead_time, color = is_canceled)) +
      geom_histogram()
    ```
...
```{r}
exp(posterior_interval(hotel_model, prob = 0.8))
```

5.  Use `classification_summary` to interpret model accuracy

```{r}
classification_summary(hotel_model, hotel_data)
```
...
```{r}
classification_summary(hotel_model, hotel_data, cutoff = 0.5)
```
